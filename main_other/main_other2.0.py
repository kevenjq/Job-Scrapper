...
def scrape(url, client):
    ...
    try:
        ...
        print(colored("Connection failed. Please check your connection and try again.\n"), "red")
        print(e)
...
def save_to_csv(filename, data):
    ...
    print(colored(f"Job listings saved to {filename}", "green"))
...
def loop_through_pages():
    ...
    pattern = re.compile(r'^[a-zA-Z\s]+,\s[a-zA-Z]{2}$')
    place = input('Enter the location. Format = City, Initials (eg: New York, NY): ').strip()

    while not pattern.match(place):
        print(colored("Please supply location in the correct format: Format = City, Initials (eg: London, OH)", "red"))
        place = input('Enter the location. Format = City, Initials (eg: New York, NY): ').strip()
    ...
    for page in range(no_of_pages):
        ...
        progress = (page + 1) / no_of_pages
        bar_length = 20
        bar = '=' * int(bar_length * progress)
        spaces = ' ' * (bar_length - len(bar))

        print(colored(f'\rProgress: [{bar}{spaces}] {int(progress * 100)}%', "green"), end='', flush=True)
        time.sleep(0.1)
